...
def generate_random_irradiation():    
    return random.choice(PV_SHAPES_POSSIBILITIES)
...
if __name__ == '__main__':
  ...
  irradiation = generate_random_irradiation()
  ...
  print(f'Residences with EV: {residences_ev}')
  print(f'Residences with PV: {residences_pv}')
  print(f'Irradiation: {irradiation}')
  print(f'PV powers: {pv_powers_residences}')
  print(f'EV chargers powers: {ev_chargers_powers}')
  print(f'EV loadshapes: {ev_loadshapes}')
  print(f'EV phases: {ev_phases}')
  print(f'PV phases: {pv_phases}')
  exit()
  dss.NewContext()
  # Load base file
  ...
  for i in range (EV_RESIDENCES_QUANTITY):
    ...
    dss.Text.Command(f'New Loadshape.shape_ev_{residences_ev[i]} npts=1440 minterval=1 mult=(file=./data/electrical_vehicles/ev_shapes_charge.csv, col={ev_loadshapes[i]})')
    dss.Text.Command(
